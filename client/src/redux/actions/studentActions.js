...
export const getTestResult = (department, year, section) => async (dispatch) => {
  try {
    console.log("Dispatching getTestResult...");
    const formData = { department, year, section };

    const { data } = await api.getTestResult(formData); // Fetch data

    console.log("Received Data:", data); // Debugging the received data

    dispatch({ type: TEST_RESULT, payload: data.result }); // Dispatching the result to the store
  } catch (error) {
    console.error("Error in getTestResult:", error.response?.data || error.message);
    dispatch({ type: SET_ERRORS, payload: error.response?.data || { message: "Unknown error" } });
  }
};
